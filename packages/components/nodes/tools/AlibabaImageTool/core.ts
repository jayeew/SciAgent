...
import {
    AlibabaImageModel,
    MAX_ALIBABA_IMAGE_COUNT,
    MAX_ALIBABA_IMAGE_SEED,
    MAX_ALIBABA_REFERENCE_IMAGES,
    MIN_ALIBABA_IMAGE_COUNT,
...
export const AlibabaImageRequestSchema = z.object({
    prompt: z.string().min(1).describe('Image prompt used to generate or edit the target illustration'),
    slideIndex: z.number().int().min(0).optional().describe('Zero-based slide index to inject the generated file into'),
    size: z.string().optional().describe('Optional Alibaba image size override, for example 2048*2048'),
    imageCount: z
        .number()
        .int()
        .min(MIN_ALIBABA_IMAGE_COUNT)
        .max(MAX_ALIBABA_IMAGE_COUNT)
        .optional()
        .describe('Optional generated image count override'),
    negativePrompt: z.string().optional().describe('Optional negative prompt override'),
    promptExtend: z.boolean().optional().describe('Optional prompt extend override'),
    watermark: z.boolean().optional().describe('Optional watermark override'),
    seed: z.number().int().min(0).max(MAX_ALIBABA_IMAGE_SEED).optional().describe('Optional seed override'),
    referenceImageFileNames: z
        .array(z.string())
        .max(MAX_ALIBABA_REFERENCE_IMAGES)
        .optional()
        .describe('Optional uploaded file names used as reference images. Supports 1-3 images.')
})

export const GenerateAlibabaImagesSchema = z.object({
    prompt: z.string().optional().describe('Simple single image prompt when imageRequests is not provided'),
    imageRequests: z
        .union([z.string(), z.array(AlibabaImageRequestSchema), z.record(z.any())])
        .optional()
    ...
    imageCount: z
        ...
        .max(MAX_ALIBABA_IMAGE_COUNT)
        .optional()
        .describe('Default generated image count override for all requests'),
    negativePrompt: z.string().optional().describe('Default negative prompt override for all requests'),
    promptExtend: z.boolean().optional().describe('Default prompt extend override for all requests'),
    watermark: z.boolean().optional().describe('Default watermark override for all requests'),
    seed: z.number().int().min(0).max(MAX_ALIBABA_IMAGE_SEED).optional().describe('Default seed override for all requests'),
...
export interface IAlibabaImageRequest extends z.infer<typeof AlibabaImageRequestSchema> {}

export interface IGenerateAlibabaImagesInput extends z.infer<typeof GenerateAlibabaImagesSchema> {}

export interface IRequestParameters {
    defaultParams?: ICommonObject
    mediaModel: AlibabaImageModel
}
...
const normalizeImageRequests = (params: IGenerateAlibabaImagesInput): IAlibabaImageRequest[] => {
            ...
            ? [
                  {
                      ...
                      ...(typeof params.promptExtend === 'boolean' ? { promptExtend: params.promptExtend } : {}),
    ...
    if (Array.isArray(parsedImageRequests)) {
        return z.array(AlibabaImageRequestSchema).parse(parsedImageRequests)
    }
    ...
    if (parsedImageRequests && typeof parsedImageRequests === 'object') {
        if (Array.isArray((parsedImageRequests as Record<string, unknown>).requests)) {
            return z.array(AlibabaImageRequestSchema).parse((parsedImageRequests as Record<string, unknown>).requests)
        }

        if ('prompt' in (parsedImageRequests as Record<string, unknown>)) {
            return [AlibabaImageRequestSchema.parse(parsedImageRequests)]
        }
    }
...
const normalizeReferenceImageFileNames = (fileNames?: string[]): string[] => {
    ...
    return [
        ...new Set(
            fileNames
                .map((fileName) => fileName.replace(/^FILE-STORAGE::/, '').trim())
                .filter(Boolean)
                .slice(0, MAX_ALIBABA_REFERENCE_IMAGES)
        )
    ]
}

const buildReferenceImages = (fileNames?: string[]): IFileUpload[] | undefined => {
    const normalizedFileNames = normalizeReferenceImageFileNames(fileNames)
    if (!normalizedFileNames.length) return undefined
...
const selectReferenceItem = <T>(items: T[], selection?: IGenerateAlibabaImagesInput['referenceImageSelection']): T | undefined => {
...
const resolveReferenceImagesFromFlowState = (
    params: IGenerateAlibabaImagesInput,
...
class GenerateAlibabaImagesTool extends DynamicStructuredTool<typeof GenerateAlibabaImagesSchema> {
    private defaultParams: ICommonObject

    private mediaModel: AlibabaImageModel

    constructor(args: IRequestParameters) {
        super({
            name: 'generate_alibaba_images',
            description:
                'Generate or edit one or more images with Alibaba DashScope Qwen Image 2.0. When presentationSpec and slideIndex are provided, this tool returns an updated presentationSpec JSON string with generated imageFileNames injected into the target slides.',
            schema: GenerateAlibabaImagesSchema,
            baseUrl: '',
            method: 'POST',
            headers: {}
        })
    ...
    protected async _call(arg: z.output<typeof GenerateAlibabaImagesSchema>, _: unknown, flowConfig?: IToolFlowConfig): Promise<string> {
        const params = {
            ...this.defaultParams,
            ...arg
        } as IGenerateAlibabaImagesInput
        ...
        try {
            ...
            for (const request of requests) {
                try {
                    ...
                    const result = await this.mediaModel.invoke(
                        {
                            ...
                            ...(effectiveRequest.size || params.size
                                ? { size: normalizeAlibabaImageSize(effectiveRequest.size || params.size) }
                                : {}),
                            ...(effectiveRequest.imageCount !== undefined || params.imageCount !== undefined
                                ? { imageCount: normalizeAlibabaImageCount(effectiveRequest.imageCount ?? params.imageCount) }
                                : {}),
                            ...(effectiveRequest.negativePrompt || params.negativePrompt
                                ? { negativePrompt: effectiveRequest.negativePrompt || params.negativePrompt }
                                : {}),
                            ...(effectiveRequest.promptExtend !== undefined || params.promptExtend !== undefined
                                ? { promptExtend: normalizeAlibabaPromptExtend(effectiveRequest.promptExtend ?? params.promptExtend) }
                                : {}),
                            ...(effectiveRequest.watermark !== undefined || params.watermark !== undefined
                                ? { watermark: normalizeAlibabaWatermark(effectiveRequest.watermark ?? params.watermark) }
                                : {}),
                            ...(effectiveRequest.seed !== undefined || params.seed !== undefined
                                ? { seed: normalizeAlibabaSeed(effectiveRequest.seed ?? params.seed) }
                                : {}),
                    ...
                    if (!fileAnnotations.length) {
                        throw new Error('No stored image file was returned by Alibaba image generation')
                    }
            ...
            return formatToolError(`Error generating Alibaba images: ${error}`, params)
        }
    }
}

export const createAlibabaImageTools = (args: IRequestParameters): DynamicStructuredTool[] => {
    return [new GenerateAlibabaImagesTool(args)]
}
